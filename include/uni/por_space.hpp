	...
	namespace Uniproc {
		...
		protected:
			...
			void schedule_eligible_successors(const State &s, const Interval<Time> &next_range, bool &found_at_least_one) override
			{
				...
				if (eligible_successors.size() > 1) {
					//Hier wordt de reduction set pas gemaakt, verder gebeurt er niet heel veel spannends
					Reduction_set<Time> reduction_set = create_reduction_set(s, eligible_successors);
			...
			//hier wordt een volledige reductie set gescheduled
			...
			Reduction_set<Time> create_reduction_set(const State &s, typename Reduction_set<Time>::Job_set &eligible_successors)
			{
				...
				//this creates the instance of the reduction set
				//maak de initiele reductie set met de earliest finish time en latest finish time van de state, de directe successors, indices zijn de indexes van de directe successors en precedence voor later.
				...
				while (true) {
					//check if there are deadline misses, if so increase the failure counter
					if (reduction_set.has_potential_deadline_misses()) {
						reduction_failures++;
